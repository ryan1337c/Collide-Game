...
public class GamePlay2 extends JPanel implements KeyListener, ActionListener, MouseListener {
	...
	private JLabel showLevel2; // label for game level
	...
	public GamePlay2() {
		...
		ballVelX = -1;
		ballVelY = -1;
		delay = 2; // delay set to 2 milliseconds
		mapBrick = new MapGenerator(row, col, 2);
	...
	public void paintComponent(Graphics c) {
		...
		showLevel2.setForeground(Color.YELLOW);
		showLevel2.setFont(new Font("Arial", Font.BOLD, 35));
		showLevel2.setText("Level 2");
		showLevel2.setBounds(10, 670, 120, 100);

		// If player loses, erases map and displays post game menu
		if (ballY > 750) {
			isPlayerPlaying = false;
			// deletes everything on the game play screen
			this.remove(showLevel2);
			...
			try { // try catch to handle any IOException errors
				int finalScore = ScoreInputOutput.getScore(score, 2); // calls getScore() get player's final score
	...
	public void actionPerformed(ActionEvent e) {
		if (isPlayerPlaying == true) {
			...
			A: for (int i = 0; i < mapBrick.getMapRow(); i++) {
				for (int k = 0; k < mapBrick.getMapCol(); k++) {
					if (mapBrick.getMapValue()[i][k] == 0) {
						ball = new Rectangle(ballX, ballY, 20, 20);
						rect = mapBrick.getRect(k, i, 2); // Draws each rectangle on the map field for each iteration
						// Detects if the ball has intersected with a brick
						if (ball.intersects(rect)) {
							...
							break A; // breaks out and goes back to outer loop to check again
						}
					}
				}
			}
		...
		if (score == 20 && isPlayerPlaying == false) {
			...
			Main.gamePlayFrame.dispose(); // deletes the current JFrame (level 2)
			Main.gamePlayFrame = new JFrame(); // Created a new JFrame for the GamePlay3 class (level 3)
			...
			GamePlay3 level3 = new GamePlay3(); // calls GamePlay2 to initialize level 3
			Main.gamePlayFrame.add(level3);
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getSource() == restartBtn) {
			...
			ballVelY = -1;
			score = 0;
			this.add(showLevel2); // adds back the level label and score label
			this.add(showScore);
			giveUpBtn.setVisible(false);
			restartBtn.setVisible(false);
